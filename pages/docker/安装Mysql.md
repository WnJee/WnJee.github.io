...
docker-compose up -d
```
>查看日志： docker-compose logs mysql <br/>
>关闭： docker-compose down <br/>
...
use admin;
source /var/lib/mysql/admin.sql
```
...
#### 修改mysql配置并重新运行容器
> 解决数据库乱码,在[mysqld] 字段里加入<br/> **character-set-server=utf8mb4** <br/>
设置表名不区分大小写 <br/> **lower_case_table_names=1**
...
mysqldump -uroot -pqaz123wsx -B itboys > /backups/mysql/itboys_backups_$(date +%Y%m%d).sql
exit
EOF
...
docker cp mysql:/backups/mysql/itboys_backups_$(date +%Y%m%d).sql /backups/mysql/
rm -f /backups/mysql/itboys_backups_$(date -d -10day +%Y%m%d).sql
...
0 3 * * * /dongshan/soft/mysql/backup.sh
```

## 配置mysql读写分离
#### 修改每个mysql配置文件[mysqld]
```
[mysqld]
log-bin=mysql-bin
...
flush privileges;
#查看master状态 （这里是 mysql-bin.000001）
show master status;
```
#### 查看master库ip （这里是172.17.0.2）
```
...
```
stop slave;
change master to master_host='172.17.0.2', master_user='slave', master_password='123456', master_log_file='mysql-bin.000001';
start slave;
show slave status;
```
> slave执行sql的时候把 **master_host** 和 **master_log_file** 替换成你自己显示的值
...
password=qaz123wsx
#将要备份的数据库
database_name=tuanxo

#保存备份文件最多个数
count=10
#备份保存路径
backup_path=/project/mysql_backup
#日期
